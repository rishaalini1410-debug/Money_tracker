...
if view_mode == "Dashboard":
    ...
    # Deduct allocated emergency & fixed savings from income for spendable balance
    ...
    if budget_cats:
        ...
        for idx, cat in enumerate(budget_cats):
            ...
            has_budget = allocated > 0.0  # Only treat as budgeted if allocation > 0
            spent = exp_df[exp_df["category"] == cat]["amount"].sum() if not exp_df.empty else 0.0

            with cat_cols[idx % num_cols]:
                if not has_budget:
                    # No active budget (> 0) set -> Show positive spent amount
                    st.markdown(f"""
                        <div class="metric-card">
                            <div class="metric-title">{cat.upper()}</div>
                            <div class="metric-value-accent">RM {spent:,.2f}</div>
                            <small style='color: #888888;'>Spent (No Budget Set)</small>
                        </div>
                    """, unsafe_allow_html=True)
                else:
                    # Active budget set -> Show remaining budget (turns negative if overspent)
                    ...
                    st.markdown(f"""
                        <div class="metric-card">
                            ...
                            <small style='color: #888888;'>Allocated: RM {allocated:,.2f}</small>
                        </div>
                    """, unsafe_allow_html=True)
    ...
    with btn_col1:
        show_summary = st.button("👀 Summary Table", use_container_width=True)
    ...
    if show_summary:
        ...
        # 2. Bar Chart: Category Allocations vs Actual Spending
        st.markdown("##### 📈 Category Spending vs Allocation")

        summary_data = []
        for cat in available_categories:
            alloc_val = alloc_df[alloc_df["category"] == cat]["allocated_amount"].sum() if not alloc_df.empty else 0.0
            spent_val = exp_df[exp_df["category"] == cat]["amount"].sum() if not exp_df.empty else 0.0
            summary_data.append({"Category": cat, "Allocated": alloc_val, "Spent": spent_val})
        ...
        if not exp_df.empty:
            st.dataframe(exp_df[["date", "category", "description", "amount"]], use_container_width=True)
        else:
    ...
    with tab1:
        ...
        with st.form("budget_form"):
            ...
            if save_budget:
                ...
                # Clear existing for selected period
